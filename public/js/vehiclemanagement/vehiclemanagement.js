document.addEventListener('DOMContentLoaded', function () {
  ...
  if (confirmArchive) {
    confirmArchive.addEventListener('click', async function () {
      ...
      try {
        ...
        if (!resp.ok) throw new Error(await resp.text());
        const data = await resp.json();
        // Show success notification using notification component
        if (typeof window.showNotification_archived_toast === 'function') {
          window.showNotification_archived_toast();
        } else {
  ...
  window.confirmValidUntil = function() {
    ...
    .then(data => {
      if (data.message) {
        // Show success notification using notification component
        if (typeof window.showNotification_validity_date_toast === 'function') {
          window.showNotification_validity_date_toast();
        } else {
          showToast(data.message, 'success');
        }
  ...
  window.confirmDecline = function() {
    ...
    .then(data => {
      if (data.message) {
        // Show success notification using notification component
        if (typeof window.showNotification_decline_toast === 'function') {
          window.showNotification_decline_toast();
        } else {
          showToast(data.message, 'success');
        }
        // Close modal and reload page after delay
  ...
  function showToast(message, type = 'success') {
    ...
    const toast = Toastify({
      ...
      gravity: "top",
      position: "right",
      className: "toastify-content notification-popup toastify-right",
    ...
    // Force right positioning after toast is shown - ensure it stays on right
    setTimeout(() => {
      ...
      if (toastElement) {
        ...
        toastElement.classList.add('toastify-right', 'notification-popup');
      }
    }, 0);
  }
