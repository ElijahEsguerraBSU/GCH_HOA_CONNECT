document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize Tom Select for nationality dropdowns
    function initializeNationalitySelects() {
        // Elements with tom-select class are automatically initialized by tom-select.js
        // But we need to re-trigger initialization for dynamically shown modals
        if (typeof $ !== 'undefined') {
            $('#nationality, #edit_nationality').each(function() {
                // Check if already initialized
                if (!this.tomselect) {
                    // Trigger automatic initialization by simulating the tom-select.js logic
                    let options = {
                        plugins: {
                            dropdown_input: {}
                        }
                    ...
                    // Add nationality-specific options
                    options.maxOptions = null;
                    options.maxItems = 1;
                    options.dropdownParent = 'body';
                    options.dropdownClass = 'nationality-dropdown';
                    
                    // Access TomSelect from window if available, or use global
                    const TomSelectConstructor = window.TomSelect || (typeof TomSelect !== 'undefined' ? TomSelect : null);
                    
                    if (TomSelectConstructor) {
                        var instance = new TomSelectConstructor(this, options);
                        ...
                        if (this.id) {
                            ...
                            window.tomSelectInstances[this.id] = instance;
                        }
                    }
                }
            });
    ...
    // Initialize when modal opens (using Tailwind modal events)
    const addModal = document.getElementById('add-landlord-modal');
    if (addModal) {
        addModal.addEventListener('show.tw.modal', function() {
            setTimeout(initializeNationalitySelects, 100);
        });
    }

    const editModal = document.getElementById('edit-landlord-modal');
    if (editModal) {
        editModal.addEventListener('show.tw.modal', function() {
            setTimeout(initializeNationalitySelects, 100);
        });
    }

    // Also initialize on page load if modals are already visible
    setTimeout(initializeNationalitySelects, 500);
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (nationalitySelect && nationalitySelect.tomselect) {
                        nationalitySelect.tomselect.destroy();
                    }
    ...
    function populateEditForm(landlord) {
        ...
        // Set nationality value for Tom Select
        const editNationalitySelect = document.getElementById('edit_nationality');
        if (editNationalitySelect) {
            if (editNationalitySelect.tomselect) {
                editNationalitySelect.tomselect.setValue(landlord.nationality || '');
            } else {
                editNationalitySelect.value = landlord.nationality || '';
            }
        }
        ...
        editForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    // Destroy Tom Select instance if exists
                    const editNationalitySelect = document.getElementById('edit_nationality');
                    if (editNationalitySelect && editNationalitySelect.tomselect) {
                        editNationalitySelect.tomselect.destroy();
                    }
