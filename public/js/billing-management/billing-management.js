...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initializeSelectAllUsers();
});

function initializeUserSelectHandler() {
    // Monitor user select changes for auto-populating billing date range
    const userSelect = document.querySelector('select[name="user_id[]"], select[name="user_id"]');
    if (userSelect) {
        // For multi-select, we'll use the Tom Select instance
        setTimeout(function() {
            const tomSelectInstance = userSelect.tomselect;
            if (tomSelectInstance) {
                tomSelectInstance.on('change', function(values) {
                    // For multi-select, use the first selected user for date range
                    if (Array.isArray(values) && values.length > 0) {
                        fetchAndSetUserBillingDateRange(values[0]);
                    } else if (values && !Array.isArray(values)) {
                        fetchAndSetUserBillingDateRange(values);
                    } else {
...
function initializeSelectAllUsers() {
    const selectAllBtn = document.getElementById('selectAllUsersBtn');
    const userSelect = document.getElementById('userSelect');
    
    if (!selectAllBtn || !userSelect) {
        console.log('Select All button or user select not found');
        return;
    }
    ...
    selectAllBtn.addEventListener('click', function() {
        ...
        setTimeout(function() {
            ...
            if (tomSelectInstance) {
                // Get all available options (excluding empty option)
                const allOptions = Object.keys(tomSelectInstance.options).filter(key => {
                    return key !== '' && tomSelectInstance.options[key].value !== '';
                });
                
                if (allOptions.length === 0) {
                    showToast('No users available to select', 'error');
                    return;
                }
                ...
                const allSelected = allOptions.every(opt => {
                    if (Array.isArray(currentValues)) {
                        return currentValues.includes(opt);
                    }
                    return currentValues === opt;
                });
                
                // Check if it's multi-select mode
                const isMultiSelect = userSelect.hasAttribute('multiple') || tomSelectInstance.settings.mode === 'multi';
                
                if (allSelected && isMultiSelect) {
                    // Deselect all
                    tomSelectInstance.clear();
                    selectAllBtn.innerHTML = `
                        ...
                        Select All
                    `;
                    showToast('All users deselected', 'info');
                } else {
                    // Select all
                    if (isMultiSelect) {
                        // Multi-select mode - select all options
                        tomSelectInstance.setValue(allOptions);
                        selectAllBtn.innerHTML = `
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                                <polyline points="9 11 12 14 22 4"></polyline>
                                <path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path>
                            ...
                            Deselect All
                        `;
                        showToast(`${allOptions.length} user(s) selected`, 'success');
                    } else {
                        // Single-select mode - select the first available user
                        if (allOptions.length > 0) {
                            tomSelectInstance.setValue(allOptions[0]);
                            showToast('User selected', 'info');
                        }
                    }
                }
            } else {
                // Fallback: if Tom Select is not initialized, try native select
                const options = userSelect.querySelectorAll('option[value]:not([value=""])');
                if (options.length > 0) {
                    // For single select, select the first option
                    userSelect.value = options[0].value;
    ...
    console.log('Select All users button initialized');
}
...
function handleCreateBilling() {
    ...
    // Validate form data
    // Handle both single user_id and array user_id[]
    const userIds = formData.getAll('user_id[]');
    const userId = formData.get('user_id'); // Fallback for single select
    const selectedUserIds = userIds.length > 0 ? userIds : (userId ? [userId] : []);
    ...
    .then(data => {
        ...
        // Log notification info if available
        if (data.notifications_sent && data.notifications_sent.length > 0) {
            console.log('Notifications sent to users:', data.notifications_sent);
            console.log('Total notifications sent:', data.notifications_count);
        }
        
        // Trigger immediate notification polling for users who received notifications
