    ...
    setTimeout(function() {
        ...
        if (createUserSelect && createUserSelect.tomselect) {
            createUserSelect.tomselect.on('change', function(value) {
                // Skip auto-selection if "all" is selected
                if (value && value !== 'all') {
...
function handleCreatePermissionSetting() {
    ...
    // Validate that if "All Users" is selected, a role must be selected
    const selectedUserId = formData.get('users_id');
    const selectedRole = document.getElementById('createRoleFilter') ? document.getElementById('createRoleFilter').value : '';
    if (selectedUserId === 'all' && !selectedRole) {
        showToast('Please select a role first when choosing "All Users"', 'error');
        return;
    }
...
// Initialize role-based auto-check for permissions
function initializeRoleBasedAutoCheck() {
    console.log('Initializing role-based auto-check for permissions...');
    // Note: The role filter change handler is set up in the blade file
    // This function is kept for potential future use or edit modal
}

// Handle role filter change and auto-check permissions based on role
function handleRoleFilterChange(role, formType) {
    const roleLower = role.toLowerCase().trim();
    console.log('Role selected for auto-check:', role);
    
    // Check if role is admin - check all permissions
    if (roleLower === 'admin') {
    ...
    // Check if we have permissions configured for this role
        ...
        console.log('No permissions configured for role:', role);
    }
}

// Clear all permission checkboxes
function clearAllPermissions(formType) {
    // Determine which container to use based on form type
    const container = formType === 'create' 
        ? document.querySelector('#createPermissionSettingForm .max-h-48.overflow-y-auto')
        : document.getElementById('editPermissionsContainer');
    
    if (!container) {
        console.log('Permissions container not found for clearing');
        return;
    }
    ...
    console.log('All', checkboxes.length, 'permissions cleared');
}
