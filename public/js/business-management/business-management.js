    ...
    table.addEventListener('click', async function (e) {
      ...
      if (editBtn) {
        try {
          ...
          // Handle business clearance display (supporting both single file and multiple files)
          ...
          if (data.business_clearance) {
            ...
            try {
              ...
              const files = JSON.parse(data.business_clearance);
              if (Array.isArray(files) && files.length > 0) {
                // Multiple files
                if (files.length === 1) {
                  currentClearanceLink.href = '/storage/' + files[0];
                  ...
                  const linksHtml = files.map((file, index) => 
                    `<a href="/storage/${file}" target="_blank" class="text-blue-600 hover:text-blue-800 underline block mb-1">
                      ...
                      View Clearance ${index + 1}
                    </a>`
                  ).join('');
                  currentClearanceDiv.innerHTML = `
          ...
          if (Array.isArray(files) && files.length > 0) {
            ...
            const linksHtml = files.map((file, index) => {
              ...
              return `<div class="mb-2">
                <a href="/storage/${file}" target="_blank" class="text-blue-600 hover:text-blue-800 underline inline-flex items-center">
                  ...
                  View Document ${index + 1}: ${fileName.length > 40 ? fileName.substring(0, 40) + '...' : fileName}
                </a>
              </div>`;
            }).join('');
            ...
            clearanceFile = `<div class="mb-2">
              <a href="/storage/${business.business_clearance}" target="_blank" class="text-blue-600 hover:text-blue-800 underline inline-flex items-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                  <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
                  <polyline points="14,2 14,8 20,8"></polyline>
                  <line x1="16" y1="13" x2="8" y2="13"></line>
                ...
                View Document: ${fileName.length > 40 ? fileName.substring(0, 40) + '...' : fileName}
              </a>
            </div>`;
          }
        } catch (e) {
      ...
      detailsContainer.innerHTML = `
        <div class="grid grid-cols-12 gap-4">
          ...
          <div class="col-span-12">
            ...
            <div class="form-control bg-slate-50 min-h-[60px] p-3">${clearanceFile}</div>
          </div>
          <div class="col-span-12 md:col-span-6">
...
// Handle Application For Certification Template Upload
document.addEventListener('DOMContentLoaded', function() {
  const uploadCertificationTemplateForm = document.getElementById('uploadCertificationTemplateForm');
  if (uploadCertificationTemplateForm) {
    uploadCertificationTemplateForm.addEventListener('submit', async function(e) {
      e.preventDefault();
      try {
        const formData = new FormData(uploadCertificationTemplateForm);
        const csrf = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
        const resp = await fetch('/business-management/upload-certification-template', {
          method: 'POST',
          headers: { 'X-Requested-With': 'XMLHttpRequest', 'X-CSRF-TOKEN': csrf },
          body: formData
        });
        ...
        // Show success notification
        if (typeof window.showNotification_users_toast_success === 'function') {
          const messageSlot = document.getElementById('users-error-message-slot');
          if (messageSlot) {
            messageSlot.textContent = data.message || 'Template uploaded successfully';
          }
          window.showNotification_users_toast_success();
        }
        
        // Close the modal
        const modal = document.getElementById('upload-certification-template-modal');
        if (modal) {
          const dismissBtn = modal.querySelector('[data-tw-dismiss="modal"]');
          if (dismissBtn) dismissBtn.click();
        }
        ...
        if (typeof window.showNotification_users_toast_error === 'function') {
          const messageSlot = document.getElementById('users-error-message-slot');
          if (messageSlot) {
            messageSlot.textContent = err.message || 'Failed to upload template';
          }
          window.showNotification_users_toast_error();
        }
      }
