...
function loadNotificationSettingForEdit(settingId) {
    ...
    .then(data => {
        ...
        // Get all enabled module IDs (from all settings for this user)
        const enabledModuleIds = data.enabledModuleIds || [];
        
        // Use the status from all settings (if any are active, use active)
        const status = data.status || setting.status;
        ...
        // Check all checkboxes for modules that the user has enabled
        enabledModuleIds.forEach(moduleId => {
            const moduleCheckbox = document.querySelector(`#editModulesContainer input[value="${moduleId}"]`);
            if (moduleCheckbox) {
...
function handleCreateNotificationSetting() {
    ...
    // Include role filter value in form submission
    ...
    if (roleFilter && roleFilterValue) {
        roleFilterValue.value = roleFilter.value || '';
        formData.set('role_filter', roleFilter.value || '');
    }
...
function initializeRoleBasedAutoCheck() {
    ...
    if (createRoleFilter) {
        ...
        createRoleFilter.addEventListener('change', function() {
            ...
            // If a specific role is selected, default Select User to "All Users"
            const userSelect = document.getElementById('createUserSelect');
            if (userSelect && userSelect.tomselect) {
                if (selectedRole) {
                    userSelect.tomselect.setValue('all');
                } else if (userSelect.tomselect.getValue() === 'all') {
                    // When clearing role filter, reset user selection back to empty
                    userSelect.tomselect.clear();
                }
            }
...
function autoCheckAllModules(formType) {
    ...
    if (checkedCount > 0) {
        showToast('Modules auto-selected for GCH-HOA President role', 'success');
    }
}
