    ...
    async function login() {
        ...
        try {
            ...
            if (data && data.success) {
                ...
                // Prevent duplicate notifications from polling system for a longer period
                if (typeof window !== 'undefined') {
                    window.__muteNotificationToastsUntil = Date.now() + 30000; // Mute for 30 seconds to prevent welcome notification
                    window.__loginJustCompleted = true; // Flag to indicate login just completed
                }
                ...
                setTimeout(() => {
                    ...
                    // Clear the login flag after redirect (will be set again on dashboard if needed)
                    setTimeout(() => {
                        window.__loginJustCompleted = false;
                    }, 5000); // Clear flag 5 seconds after redirect
