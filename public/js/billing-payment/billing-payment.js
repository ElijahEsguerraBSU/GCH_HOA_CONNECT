...
function loadBillingDetails(billingId) {
    ...
    fetch(`/billing-payment/${billingId}`, {
        method: 'GET',
        headers: {
    ...
    .then(response => {
        if (!response.ok) {
            return response.json()
                .then(errorData => {
                    const apiMessage = errorData && errorData.message ? errorData.message : '';
                    const error = new Error(apiMessage || 'Failed to load billing details.');
    ...
    .then(data => {
        if (!data || !data.billing) {
            throw new Error('Billing details response is incomplete.');
        }
    ...
    .catch(error => {
        ...
        const message = error && error.message 
            ? error.message 
            : 'Error loading billing details. Please try again.';
...
function displayBillingDetails(billing) {
    ...
    billing.billing_items.forEach((item, index) => {
        ...
        itemsHtml += `
            <tr>
                <td class="border-b border-slate-200 py-3 px-4 whitespace-nowrap">${index + 1}</td>
                <td class="border-b border-slate-200 py-3 px-4 whitespace-nowrap">${item.description}</td>
                <td class="border-b border-slate-200 py-3 px-4 text-center whitespace-nowrap">${item.qty}</td>
                <td class="border-b border-slate-200 py-3 px-4 text-right whitespace-nowrap">₱${parseFloat(item.price).toFixed(2)}</td>
                <td class="border-b border-slate-200 py-3 px-4 text-right font-medium whitespace-nowrap">₱${total}</td>
            </tr>
        `;
    });
        ...
        <div class="p-6">
            ...
            <div class="mt-8">
                <div class="bg-orange-50 p-6 rounded-lg border border-orange-200">
                    ...
                    <div class="bg-white rounded-lg border overflow-x-auto" style="-webkit-overflow-scrolling: touch;">
                        <table class="w-full min-w-[600px]">
                            <thead class="bg-slate-50">
                                <tr>
                                    <th class="text-left py-3 px-4 font-semibold text-slate-700 whitespace-nowrap">#</th>
                                    <th class="text-left py-3 px-4 font-semibold text-slate-700 whitespace-nowrap">Description</th>
                                    <th class="text-center py-3 px-4 font-semibold text-slate-700 whitespace-nowrap">Qty</th>
                                    <th class="text-right py-3 px-4 font-semibold text-slate-700 whitespace-nowrap">Price</th>
                                    <th class="text-right py-3 px-4 font-semibold text-slate-700 whitespace-nowrap">Total</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${itemsHtml}
                                <tr class="bg-slate-100 font-bold">
                                    <td colspan="4" class="py-4 px-4 text-right whitespace-nowrap">TOTAL AMOUNT DUE:</td>
                                    <td class="py-4 px-4 text-right text-lg whitespace-nowrap">₱${parseFloat(billing.amount_due).toFixed(2)}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
...
function displayReceiptFile(receiptPath, receiptType = 'user') {
    ...
    const iframeHeight = isMobile ? '500px' : '600px';
    const iframeMinHeight = isMobile ? '400px' : '600px';
    
    if (receiptType === 'official' && fileExtension === 'html') {
        // Display official HTML receipt in iframe with responsive height and horizontal scroll
        receiptDisplay.innerHTML = `
            <div class="receipt-iframe-container" style="width: 100%; overflow-x: auto; overflow-y: visible; -webkit-overflow-scrolling: touch; min-width: 100%;">
                <iframe 
                    src="${fileUrl}" 
                    class="border rounded-lg shadow-lg receipt-iframe" 
                    style="height: ${iframeHeight}; min-height: ${iframeMinHeight}; min-width: 600px; width: 100%; max-width: none; border: 1px solid #e5e7eb; display: block;"
                    scrolling="auto"
                    frameborder="0">
                </iframe>
            </div>
        ...
        // Adjust iframe height after load for better mobile experience
        setTimeout(() => {
            const iframe = receiptDisplay.querySelector('iframe');
            if (iframe) {
                iframe.onload = function() {
                    try {
                        // Try to get content height (may fail due to CORS)
                        ...
                        // CORS issue, use default responsive height
        ...
        // Display image with horizontal scroll support
        receiptDisplay.innerHTML = `
            <div class="text-center" style="overflow-x: auto; -webkit-overflow-scrolling: touch;">
                <img src="${fileUrl}" alt="Payment Receipt" class="h-auto mx-auto rounded-lg shadow-lg" style="max-height: 70vh; min-width: auto; width: auto; max-width: none;">
                <p class="text-sm text-slate-500 mt-3">Payment Receipt Image</p>
            </div>
        `;
    } else if (fileExtension === 'pdf') {
        // Display PDF with responsive height and horizontal scroll
        receiptDisplay.innerHTML = `
            <div class="text-center" style="overflow-x: auto; -webkit-overflow-scrolling: touch;">
                <iframe 
                    src="${fileUrl}" 
                    class="border rounded-lg shadow-lg" 
                    style="height: ${iframeHeight}; min-height: ${iframeMinHeight}; min-width: 600px; width: 100%; max-width: none; display: block;">
                </iframe>
...
function setupDownloadButton(receiptPath) {
    ...
    downloadBtn.onclick = async function() {
        // If it's an HTML file, convert to image
        if (fileExtension === 'html') {
            try {
                const receiptDisplay = document.getElementById('receiptFileDisplay');
                const iframe = receiptDisplay.querySelector('iframe');
                
                if (iframe && typeof html2canvas !== 'undefined') {
                    showToast('Converting receipt to image...', 'info');
                    ...
                    // Fallback: download as HTML if html2canvas not available
                    const link = document.createElement('a');
                    link.href = fileUrl;
                ...
                showToast('Error converting receipt. Downloading as HTML...', 'error');
            ...
            // For non-HTML files (images, PDFs, etc.), download as-is
            const link = document.createElement('a');
            link.href = fileUrl;
            link.download = fileName;
            document.body.appendChild(link);
