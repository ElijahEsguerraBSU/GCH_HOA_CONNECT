...
(function checkAndInit() {
    ...
    $(document).ready(function() {
        ...
        const formatAppointmentsForCalendar = (appointments) => {
            ...
            if (appointmentTime && typeof appointmentTime === 'string' && appointmentTime.trim() !== '') {
                ...
                try {
                    ...
                    if (timeParsed && !isNaN(hours) && !isNaN(minutes) && hours >= 0 && hours <= 23 && minutes >= 0 && minutes <= 59) {
                        ...
                        console.warn('Invalid time values from Scheduled Time field, using default time of 00:00:00');
                        console.warn('Parsed values:', { hours, minutes, seconds, original: timeStr });
                        // Use default time if parsing failed but time exists
                    ...
                    console.warn('Using default time (00:00:00) due to parsing error');
                    startDateTime = datePart + 'T00:00:00';
                    hasTime = true;
                }
            } else {
                // No Scheduled Time / Appointment Time specified, assign default time (9:00 AM) so it appears in time slots
                // Since allDaySlot is false, we need all events to have a time
            ...
            // If hasTime is true, startDateTime MUST include 'T' separator and time
            // FullCalendar uses this to determine if it's an all-day event
            if (hasTime && !startDateTime.includes('T')) {
                console.warn('Time exists but startDateTime missing T separator, fixing:', startDateTime);
                startDateTime = datePart + 'T00:00:00';
            }
            
            // CRITICAL: ALL events must be timed events (allDay: false) to appear in time slots
            // This ensures appointments appear at their scheduled times like Google Calendar
            // NEVER set allDay to true - we want all events in time slots
            // Example: Nov 20, 12:00 PM should appear at 12:00 PM row, NOT in all-day section
            const isAllDay = false; // ALWAYS false - all events must appear in time slots
            
            // CRITICAL: Ensure startDateTime always has time component (never just date)
            // If somehow we only have a date, add default time to ensure it's not treated as all-day
            if (!startDateTime.includes('T')) {
                console.warn('startDateTime missing time component, adding default time:', startDateTime);
                startDateTime = datePart + 'T09:00:00'; // Default to 9 AM if time is missing
                hasTime = true;
            }
            
            // CRITICAL: Calculate end time for timed events (FullCalendar needs this)
            // Default duration is 1 hour for appointments
            let endDateTime = null;
            if (startDateTime.includes('T')) {
                const startDate = new Date(startDateTime);
                // Add 1 hour as default duration
                const endDate = new Date(startDate.getTime() + 60 * 60 * 1000);
                endDateTime = endDate.toISOString().slice(0, 19); // Format: YYYY-MM-DDTHH:mm:ss
            }
            ...
            const eventObj = {
                ...
                end: endDateTime, // End time ensures FullCalendar treats this as a timed event
                allDay: false, // CRITICAL: ALWAYS false - all events must appear in time slots, not all-day
            ...
            // Additional validation: warn if time exists but event is marked as all-day
            if (hasTime && eventObj.allDay) {
                console.error('ERROR: Time exists but event is marked as all-day!', {
                    appointmentId: appointmentId,
            ...
            // CRITICAL: Final validation - ensure appointmentTime is linked to calendar
            if (appointmentTime && !eventObj.start.includes('T')) {
                console.error('ERROR: appointmentTime exists but start datetime has no time component!', {
                    appointmentId: appointmentId,
                    appointmentTime: appointmentTime,
                    start: eventObj.start,
                    hasTime: hasTime
            ...
            console.log('=== APPOINTMENT TIME LINKAGE SUMMARY ===', {
                ...
                timeLinked: hasTime && eventObj.start.includes('T'),
                calendarTimeSlot: hasTime ? eventObj.start.split('T')[1] : 'N/A'
            });
        ...
        // CRITICAL: Helper function to format time from event (defined before calendar)
        function formatEventTime(event) {
            const scheduledTime = event.extendedProps?.time;
            let formattedTime = '';
            
            // Format time from event start date
            ...
            } else if (scheduledTime && typeof scheduledTime === 'string' && scheduledTime.trim()) {
                // Try to parse scheduled time
                try {
                    const timeStr = scheduledTime.trim();
                    // Check if it's already in format like "9:00 AM" or "09:00 AM"
                    if (timeStr.match(/\d{1,2}:\d{2}\s*(AM|PM)/i)) {
                        ...
                        if (h && m) {
                            ...
                            const minutes = parseInt(m.split(/\s/)[0], 10);
                            const ampm = hours >= 12 ? 'PM' : 'AM';
                            const displayHours = hours % 12 || 12;
                            formattedTime = `${displayHours}:${minutes.toString().padStart(2, '0')} ${ampm}`;
                        } else {
                    ...
                    formattedTime = scheduledTime.trim();
                }
            } else if (event.startStr && event.startStr.includes('T')) {
                // Parse from ISO string
                const timePart = event.startStr.split('T')[1];
        ...
        try {
            ...
            headerToolbar: {
                ...
                center: "title",
                right: "dayGridMonth,listWeek", // Only Month and List views
            },
            navLinks: false, // Disable day header navigation links (make days not clickable)
            editable: false,
            dayMaxEvents: true,
            initialView: 'dayGridMonth', // Set default view to Month
            initialDate: new Date(), // CRITICAL: Show today's date by default
            // CRITICAL: Global setting to remove "all-day" text
            allDayText: '', // Remove "all-day" text globally
            eventTimeFormat: {
                hour: '2-digit',
                minute: '2-digit',
                hour12: true
            },
            // Configure views - only Month and List
            views: {
                dayGridMonth: {
                ...
                listWeek: {
                    ...
                    allDayText: '', // Remove "all-day" text
                    duration: { days: 7 }, // Show 7 days (week view)
                    listDayFormat: { weekday: 'long', month: 'short', day: 'numeric' } // Format: "Monday, Nov 18"
                }
            },
            ...
            eventDidMount: function(info) {
                ...
                // CRITICAL: Fix List view to show scheduled times instead of "all-day"
                if (info.view.type === 'listWeek') {
                    // Force event to NOT be all-day
                    if (info.event.allDay) {
                        info.event.setProp('allDay', false);
                    }
                    
                    // Format time using global helper function
                    const formattedTime = formatEventTime ? formatEventTime(info.event) : '';
                    
                    // Use multiple setTimeout to catch FullCalendar's rendering
                    const updateTimeCell = () => {
                        // Find time cell in List view
                        let timeCell = info.el.querySelector('td.fc-list-event-time');
                        
                        if (!timeCell) {
                            // Try alternate selectors
                            const cells = info.el.querySelectorAll('td');
                            if (cells.length > 0) {
                                timeCell = cells[0]; // First cell is usually time
                            }
                        }
                        
                        if (timeCell && formattedTime) {
                            const currentText = (timeCell.textContent || '').trim().toLowerCase();
                            
                            // Always replace "all-day" text or empty text
                            if (currentText.includes('all-day') || 
                                currentText === '' || 
                                !currentText.match(/\d{1,2}:\d{2}\s*(am|pm)/i)) {
                                
                                timeCell.textContent = formattedTime;
                                timeCell.innerHTML = formattedTime; // Also set innerHTML to be sure
                                timeCell.style.display = 'table-cell';
                                timeCell.style.visibility = 'visible';
                                timeCell.style.opacity = '1';
                                info.el.classList.remove('fc-event-allday');
                                info.el.removeAttribute('data-all-day');
                                
                                console.log('✓ List view: Updated', info.event.title, '→', formattedTime, 'Current text was:', currentText);
                            }
                        }
                    };
                    
                    // Call multiple times to catch all render phases - MORE AGGRESSIVE
                    setTimeout(updateTimeCell, 0);
                    setTimeout(updateTimeCell, 50);
                    setTimeout(updateTimeCell, 100);
                    setTimeout(updateTimeCell, 200);
                    setTimeout(updateTimeCell, 300);
                    setTimeout(updateTimeCell, 500);
                    setTimeout(updateTimeCell, 1000);
                }
                
                console.log('Event mounted:', info.event.id, info.event.title, info.el, 'allDay:', info.event.allDay);
            },
            dateClick: function(info) {
                // Disabled - day headers are not clickable
                console.log('Date click disabled');
                return false;
            },
            drop: function (info) {
                if (
            ...
            // CRITICAL: Ensure calendar shows today's date
            const today = new Date();
            calendar.gotoDate(today);
            console.log('Calendar navigated to today:', today.toISOString().split('T')[0]);
            ...
            console.log('Current calendar date:', calendar.view.currentStart);
            ...
            // CRITICAL: Function to update List view times after calendar renders
            function updateListViewModelTimes() {
                if (!calendar || !calendar.view) return;
                
                if (calendar.view.type === 'listWeek') {
                    console.log('=== UPDATING LIST VIEW TIMES ===');
                    
                    // Get all list event rows
                    const listEvents = document.querySelectorAll('tr.fc-list-event');
                    console.log('Found', listEvents.length, 'list events');
                    
                    listEvents.forEach((eventEl) => {
                        // Get event ID
                        let eventId = eventEl.getAttribute('data-event-id');
                        
                        if (!eventId) {
                            // Try to find by title
                            const titleCell = eventEl.querySelector('.fc-list-event-title');
                            if (titleCell) {
                        ...
                        // Find time cell
                        let timeCell = eventEl.querySelector('td.fc-list-event-time');
                        if (!timeCell) {
                            const cells = eventEl.querySelectorAll('td');
                            timeCell = cells[0]; // First cell is time
                        }
                        ...
                        // AGGRESSIVE: ALWAYS replace text if it contains "all-day" or doesn't look like a time
                        if (formattedTime) {
                            const currentText = (timeCell.textContent || '').trim().toLowerCase();
                            
                            // ALWAYS replace if it says "all-day" or doesn't match time pattern
                            if (currentText.includes('all-day') || 
                                currentText === '' || 
                                !currentText.match(/\d{1,2}:\d{2}\s*(am|pm)/i)) {
                                ...
                                timeCell.setAttribute('data-time-set', 'true');
                                timeCell.style.display = 'table-cell';
                                timeCell.style.visibility = 'visible';
                                timeCell.style.opacity = '1';
                                timeCell.style.fontSize = '14px';
                                timeCell.style.color = '#475569';
                                eventEl.classList.remove('fc-event-allday');
                                eventEl.removeAttribute('data-all-day');
                                
                                console.log('✓ FORCE UPDATED:', event.title, '→', formattedTime, 'Was:', currentText);
                            }
                        } else {
                            // Even if no formatted time, hide "all-day" text
                            const currentText = (timeCell.textContent || '').trim().toLowerCase();
                            if (currentText.includes('all-day')) {
                                timeCell.textContent = '';
                                timeCell.innerHTML = '';
                                timeCell.style.fontSize = '0';
                                timeCell.style.opacity = '0';
                            }
                        }
                    });
                }
            ...
            // CRITICAL: MutationObserver to watch for changes FullCalendar makes to DOM
            const observer = new MutationObserver(function(mutations) {
                if (calendar && calendar.view && calendar.view.type === 'listWeek') {
                    let shouldUpdate = false;
                    mutations.forEach(function(mutation) {
                        if (mutation.type === 'childList' || mutation.type === 'characterData') {
                            const target = mutation.target;
                            if (target && (
                                target.classList.contains('fc-list-event-time') ||
                                target.closest('.fc-list-event-time') ||
                                target.closest('tr.fc-list-event')
                            )) {
                                const text = (target.textContent || '').trim().toLowerCase();
                                // ALWAYS update if we see "all-day" or empty text or doesn't look like time
                                if (text.includes('all-day') || text === '' || !text.match(/\d{1,2}:\d{2}\s*(am|pm)/i)) {
                                    shouldUpdate = true;
                                }
                            }
                        }
                    ...
                    if (shouldUpdate) {
                        setTimeout(updateListViewModelTimes, 50);
                        setTimeout(updateListViewModelTimes, 200);
                    }
                }
                // Also disable day headers when DOM changes
                setTimeout(disableDayHeaderClicks, 100);
            });
            
            // Start observing after calendar renders
            setTimeout(() => {
                ...
                if (calendarContainer) {
                    ...
                    console.log('MutationObserver started to watch for "all-day" text changes');
                }
            }, 1000);
            ...
            calendar.on('viewDidMount', function(viewInfo) {
                if (viewInfo.view.type === 'listWeek') {
                    setTimeout(updateListViewModelTimes, 500);
                }
            });
            
            // Update after events are loaded
            calendar.on('eventsSet', function() {
                setTimeout(() => {
                    if (calendar.view && calendar.view.type === 'listWeek') {
                        updateListViewModelTimes();
                    }
                }, 500);
            });
            
            // Initial update if List view is active
            setTimeout(() => {
                if (calendar.view && calendar.view.type === 'listWeek') {
                    updateListViewModelTimes();
                }
            ...
            // CRITICAL: AGGRESSIVE - Disable day header clicks after calendar renders
            function disableDayHeaderClicks() {
                // Disable clicks on List view day headers - MORE AGGRESSIVE
                const listDayHeaders = document.querySelectorAll('.fc-list-day, .fc-list-day-text, .fc-list-day-side-text, .fc-list-day a, .fc-list-day-text a, .fc-list-day-side-text a');
                listDayHeaders.forEach(header => {
                    ...
                    // Add capture phase listener to prevent ALL clicks
                ...
                // Disable clicks on Month view day headers - MORE AGGRESSIVE
                const monthDayHeaders = document.querySelectorAll('.fc-daygrid-day-header, .fc-col-header-cell, .fc-daygrid-day-header a, .fc-col-header-cell a');
                monthDayHeaders.forEach(header => {
                    header.style.pointerEvents = 'none';
                    header.style.cursor = 'default';
                    header.style.userSelect = 'none';
                    // Remove all click handlers
                ...
                console.log('Day headers disabled:', listDayHeaders.length + monthDayHeaders.length, 'elements');
            }
            
            // Run MULTIPLE times to catch all render phases
            setTimeout(disableDayHeaderClicks, 100);
            setTimeout(disableDayHeaderClicks, 300);
            setTimeout(disableDayHeaderClicks, 500);
            setTimeout(disableDayHeaderClicks, 1000);
            calendar.on('viewDidMount', function() {
                setTimeout(disableDayHeaderClicks, 50);
                setTimeout(disableDayHeaderClicks, 200);
                setTimeout(disableDayHeaderClicks, 500);
            });
            calendar.on('eventsSet', function() {
                setTimeout(disableDayHeaderClicks, 100);
            ...
            const calendarClickHandler = function(e) {
                ...
                // CRITICAL: AGGRESSIVE - Prevent clicks on day headers (Monday, Tuesday, etc.)
                const target = e.target;
                if (target.classList.contains('fc-list-day') ||
                    target.classList.contains('fc-list-day-text') ||
                    target.classList.contains('fc-list-day-side-text') ||
                    target.classList.contains('fc-daygrid-day-header') ||
                    target.classList.contains('fc-col-header-cell') ||
                    target.closest('.fc-list-day') ||
                    target.closest('.fc-list-day-text') ||
                    target.closest('.fc-list-day-side-text') ||
                    target.closest('.fc-daygrid-day-header') ||
                    target.closest('.fc-col-header-cell')) {
                    console.log('Day header click prevented');
                    e.preventDefault();
                    e.stopPropagation();
                    e.stopImmediatePropagation();
                    return false;
