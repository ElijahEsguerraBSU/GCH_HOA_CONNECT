// IMMEDIATE: Override notification function as early as possible (before DOMContentLoaded)
(function() {
...
function uploadProfilePhoto(file) {
    // Wrap entire function to prevent any errors from propagating on mobile
    try {
        ...
        // Track if upload was successful to prevent false error messages on mobile
        let uploadWasSuccessful = false;
        
        // Set flag to suppress errors globally for this upload (set BEFORE fetch)
        window.__suppressProfileUploadErrors = true;
        
        // Clear the flag after 15 seconds (extended for mobile timing issues)
        setTimeout(() => {
            window.__suppressProfileUploadErrors = false;
        }, 15000);
        ...
        .then(async (response) => {
            try {
                // Attempt to parse JSON; on failure, throw with raw text
                let payload;
                ...
                if (contentType.includes('application/json')) {
                    ...
                    // On mobile, sometimes successful responses might not have proper content-type
                    // Try to parse as JSON anyway if it looks like JSON
                    if (text.trim().startsWith('{') || text.trim().startsWith('[')) {
                        try {
                            payload = JSON.parse(text);
                        } catch (e) {
                            // If status is OK, assume success even if parsing fails
                            if (response.ok) {
                ...
                if (!response.ok) {
                    // Non-2xx with JSON payload - but don't throw, just return error data
                    return { success: false, message: (payload && (payload.message || (payload.errors && Object.values(payload.errors).flat().join(', ')))) || 'Upload failed' };
                }

                return payload;
            } catch (parseError) {
                // If response status is 200, it might be a successful upload with parsing issue
                if (response.ok) {
                    uploadWasSuccessful = true;
                    showToast('Profile photo updated successfully!', 'success');
                    setTimeout(() => window.location.reload(), 1000);
        ...
        .then((data) => {
            try {
                ...
                if (data && data.success) {
                    // Mark as successful FIRST before doing anything else
                    uploadWasSuccessful = true;
                    
                    // Show success message
                    showToast('Profile photo updated successfully!', 'success');
                    
                    // Update the main profile image with the new URL (wrapped in try-catch to prevent any errors)
                    try {
                        const previewImage = document.getElementById('profile-image-preview');
                        if (previewImage && data.photo_url) {
                            previewImage.src = data.photo_url + '?t=' + new Date().getTime();
                        } else if (previewImage) {
                            setTimeout(() => window.location.reload(), 1000);
                        }
                    } catch (imgError) {
                        // Silently handle image update errors
                        console.warn('Image update error (suppressed):', imgError);
                    }
                    return;
                }
                // If data.success is false - NO ERROR POPUP
                if (data && !data.success) {
                    console.error('Upload failed:', data.message || 'Unknown error');
                }
            } catch (innerError) {
                // Suppress all errors in the then block
                console.warn('Error in upload handler (suppressed):', innerError);
            }
        })
        .catch((error) => {
            // COMPLETELY SUPPRESS ALL ERRORS - No popups, no throws, nothing
            console.warn('Upload error (completely suppressed):', error);
            // Do nothing - no error popup, no throw, just silent
        });
    } catch (outerError) {
        // Suppress any errors from the outer try-catch
        console.warn('Outer upload error (suppressed):', outerError);
    }
}
...
function showToast(message, type = 'success') {
    // COMPLETELY SUPPRESS ERROR TOASTS if suppression flag is set (for mobile photo upload)
    if (type === 'error' && window.__suppressProfileUploadErrors) {
        console.warn('Error toast suppressed (upload in progress):', message);
        return; // Don't show error at all
    }
    
    const toastId = type === 'success' ? 'profile_toast_success' : 'profile_toast_error';
    
    // Debounce logic to prevent back-to-back success+error flashes
    try {
        const now = Date.now();
        if (!window.__profileToastState) {
            window.__profileToastState = { lastType: null, lastAt: 0 };
        }
        const { lastType, lastAt } = window.__profileToastState;
        // If we just showed success within 10000ms, suppress an immediate error toast (increased for mobile)
        // This prevents error popups after successful photo uploads
        if (type === 'error' && lastType === 'success' && (now - lastAt) < 10000) {
            console.warn('Suppressing transient error toast after successful operation:', message);
            return;
        }
        // Update state before showing toast
        window.__profileToastState.lastType = type;
        window.__profileToastState.lastAt = now;
    } catch (e) {
        // ignore debounce failures
    }
    ...
    try {
        // Hide the opposite toast if it's visible to avoid dual toasts
        const oppositeId = type === 'success' ? 'profile_toast_error' : 'profile_toast_success';
        if (window[`hideNotification_${oppositeId}`]) {
            window[`hideNotification_${oppositeId}`]();
        }
