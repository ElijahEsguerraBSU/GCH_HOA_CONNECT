...
<div class="col-span-12 mt-6 -mb-6 intro-y">
    <div class="alert alert-dismissible show box bg-primary text-white flex items-center mb-6" role="alert">
        <span>Permission Settings: Manage user permissions and access control. Control which modules users can access.</span>
...
<div class="intro-y col-span-12">
    <x-notification-toast id="permission_settings_toast_success" type="success" title="Success" message="Permission setting updated successfully"
        :showButton="false" />
    <x-notification-toast id="permission_settings_toast_error" type="error" title="Error" :showButton="false">
        <div id="permission_settings_error_message_slot" class="text-slate-500 mt-1"></div>
...
<div class="grid grid-cols-12 gap-6 mt-5">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <button class="btn btn-primary shadow-md mr-2" data-tw-toggle="modal" data-tw-target="#create-permission-setting-modal">
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        <table class="table table-report -mt-2">
            ...
            <tbody>
                @forelse($permissionSettings as $setting)
                <tr class="intro-x" data-status="{{ $setting->status }}" data-modules="{{ $setting->permissionSettingsList->pluck('module_id')->implode(',') }}">
                    <td class="w-40">
                    ...
                    <td class="w-40">
                        <div class="text-center">
                            @if($setting->permissionSettingsList && $setting->permissionSettingsList->count() > 0)
                                <span class="px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full font-medium">
                                    {{ $setting->permissionSettingsList->count() }} {{ $setting->permissionSettingsList->count() === 1 ? 'Permission' : 'Permissions' }}
                                </span>
                            @else
                                <span class="text-slate-500 text-sm">No permissions</span>
                            @endif
                        </div>
                    </td>
                    <td class="w-40">
                    ...
                    <td class="table-report__action w-56">
                        <div class="flex justify-center items-center">
                            <a class="flex items-center mr-3" href="javascript:;" data-tw-toggle="modal" data-tw-target="#view-permission-setting-modal" data-setting-id="{{ $setting->id }}">
                            ...
                            <a class="flex items-center mr-3" href="javascript:;" data-tw-toggle="modal" data-tw-target="#edit-permission-setting-modal" data-setting-id="{{ $setting->id }}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                            ...
                            <a class="flex items-center text-danger" href="javascript:;" data-tw-toggle="modal" data-tw-target="#delete-permission-setting-modal" data-setting-id="{{ $setting->id }}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                ...
                <tr class="intro-x">
                    <td colspan="6" class="text-center py-8">
                        <div class="text-slate-500">
                            ...
                            <div class="font-medium">No permission settings found</div>
                            <div class="text-sm">Start by adding your first permission setting</div>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <!-- BEGIN: Pagination -->
    <x-pagination 
        :current-page="$permissionSettings->currentPage()" 
        :total-pages="$permissionSettings->lastPage()" 
        :per-page="$permissionSettings->perPage()" 
...
<!-- BEGIN: Create Permission Setting Modal -->
<div id="create-permission-setting-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Add Permission Setting</h2>
            ...
            <div class="modal-body px-6 py-8">
                <form id="createPermissionSettingForm" method="POST" action="{{ route('permission-settings.store') }}">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="role_filter" id="createRoleFilterValue" value="">
                    ...
                    <div class="mb-6">
                        ...
                        <small class="text-slate-500">Choose a specific user or select <strong>All Users</strong> to apply these permission settings to all users with the selected role.</small>
                    </div>
                    
                    <div class="mb-6">
                        <label class="form-label text-base font-semibold text-slate-700">Permissions</label>
                        ...
                        <small class="text-slate-500">Select one or more modules to grant permissions</small>
                    </div>
            ...
            <div class="modal-footer px-6 py-4 bg-slate-50">
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" form="createPermissionSettingForm" class="btn btn-primary px-6 py-2 flex items-center mr-2">
...
<!-- END: Create Permission Setting Modal -->

<!-- BEGIN: View Permission Setting Modal -->
<div id="view-permission-setting-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            
            <div class="modal-body p-0">
                <div id="permission-setting-details">
...
<!-- END: View Permission Setting Modal -->

<!-- BEGIN: Edit Permission Setting Modal -->
<div id="edit-permission-setting-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto">Edit Permission Setting</h2>
                <button type="button" class="btn btn-outline-secondary w-8 h-8 mr-1" data-tw-dismiss="modal">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x w-4 h-4"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body px-6 py-8">
                <form id="editPermissionSettingForm" method="POST">
                    <input type="hidden" name="_token" value="{{ csrf_token() }}">
                    <input type="hidden" name="_method" value="PUT">
                    <input type="hidden" id="editPermissionSettingId">
                    ...
                    <div class="mb-6">
                        <label class="form-label text-base font-semibold text-slate-700">Permissions</label>
                        <div class="mt-2 p-4 border border-slate-300 rounded-lg max-h-48 overflow-y-auto" id="editPermissionsContainer">
                            @foreach($modules as $module)
                                <label class="flex items-center mb-2">
                                    <input type="checkbox" name="permissions[]" value="{{ $module->id }}" class="form-check-input mr-3">
                                    <span class="text-slate-700">{{ ucwords($module->module_name) }}</span>
            ...
            <div class="modal-footer px-6 py-4 bg-slate-50">
                <div class="flex justify-end gap-3">
                    <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary px-6 py-2 mr-2">Cancel</button>
                    <button type="submit" form="editPermissionSettingForm" class="btn btn-primary px-6 py-2 flex items-center mr-2">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                            <polyline points="9 11 12 14 22 4"></polyline>
                            <path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11"></path>
                        </svg>
...
<!-- END: Edit Permission Setting Modal -->

<!-- BEGIN: Delete Permission Setting Modal -->
<div id="delete-permission-setting-modal" class="modal" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content">
            <div class="modal-body p-0">
                <div class="p-5 text-center">
                    ...
                    <div class="text-slate-500 mt-2">Do you really want to delete this permission setting? This process cannot be undone.</div>
                </div>
                <div class="px-5 pb-8 text-center">
                    <input type="hidden" id="deletePermissionSettingId">
                    <div class="flex justify-center gap-2">
                        <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-24 mb-2">Cancel</button>
                        <button type="button" class="btn btn-danger w-24 mb-2" id="confirmDeletePermissionSetting">Delete</button>
                    </div>
                </div>
            </div>
        </div>
...
<!-- END: Delete Permission Setting Modal -->
    ...
    <script>
        ...
        function initializeCreateRoleFilter() {
            ...
            roleFilter.addEventListener('change', function() {
                ...
                const roleFilterValue = document.getElementById('createRoleFilterValue');
                if (roleFilterValue) {
                    roleFilterValue.value = selectedRole || '';
                }
                
                // Auto-check permissions based on selected role
                if (selectedRole && typeof handleRoleFilterChange === 'function') {
                    handleRoleFilterChange(selectedRole, 'create');
                } else if (!selectedRole && typeof clearAllPermissions === 'function') {
                    clearAllPermissions('create');
                }
                ...
                setTimeout(function() {
                    ...
                    if (tomSelectInstance) {
                        ...
                        const wasAllUsers = currentValue === 'all';
                        
                        // Clear existing options
                        tomSelectInstance.clearOptions();
                        
                        // Only include "All Users" option if a role is selected
                        if (selectedRole) {
                            tomSelectInstance.addOption({
                                value: 'all',
                                text: 'All Users (' + (selectedRole === 'admin' ? 'GCH-HOA President' : selectedRole.charAt(0).toUpperCase() + selectedRole.slice(1)) + ')',
                                role: ''
                            });
                        }
                        ...
                        const filteredOptions = selectedRole 
                            ? allUserOptions.filter(opt => opt.role === selectedRole && opt.value !== 'all')
                            : allUserOptions.filter(opt => opt.value !== 'all');
                        ...
                        // Clear selection if "All Users" was selected but role is now cleared
                        if (wasAllUsers && !selectedRole) {
                            tomSelectInstance.clear();
                        } else if (wasAllUsers && selectedRole) {
                            // Restore "All Users" selection if role is still selected
                            tomSelectInstance.setValue('all');
                        } else {
                            // Clear current selection when role changes
                            tomSelectInstance.clear();
                        }
