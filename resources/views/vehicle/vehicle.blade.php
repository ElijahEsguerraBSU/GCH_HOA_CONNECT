                ...
                <tr class="intro-x" data-status="{{ $vehicle->status }}">
                    ...
                    <td class="w-40">
                        <div class="flex items-center justify-center 
                            @if($vehicle->status === 'Active' || $vehicle->status === 'Approved') text-success
                            @elseif($vehicle->status === 'Pending') text-warning
                            @elseif($vehicle->status === 'Declined') text-danger
                            ...
                            @elseif($vehicle->status === 'Declined')
                                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                                    <circle cx="12" cy="12" r="10"></circle>
            ...
            <div class="modal-body px-6 py-8">
                <form id="createVehicleForm" method="POST" action="{{ route('vehicle.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        @if($hasStickerFormTemplate)
                        <div class="mb-4 mt-2 p-3 border border-slate-200 rounded-lg bg-slate-50">
                            <label class="form-label text-sm font-medium text-slate-700 mb-2">Vehicle Sticker Form</label>
                            <p class="text-sm text-slate-600 mb-2">Download the Vehicle Sticker Form, fill it out, and upload it with your other supporting documents below.</p>
                            <a href="{{ route('vehicle.download-sticker-form-template') }}" class="btn btn-outline-primary btn-sm inline-flex items-center" download>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                        ...
                        <div class="mt-4">
                            <label class="form-label text-sm font-medium text-slate-600 mb-2">{{ $hasStickerFormTemplate ? 'Upload Supporting Documents (including filled Vehicle Sticker Form)' : 'Supporting Documents' }} <span class="text-danger">*</span></label>
                            <input type="file" name="supporting_documents_attachments[]" id="createSupportingDocuments" class="form-control p-3 border border-slate-300 rounded-lg focus:border-blue-500 focus:ring-blue-500" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" multiple required>
            ...
            <div class="modal-body px-6 py-8">
                <form id="editVehicleForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="mt-4">
                            <label class="form-label text-sm font-medium text-slate-600 mb-2">{{ $hasStickerFormTemplate ? 'Upload Supporting Documents (including filled Vehicle Sticker Form)' : 'Supporting Documents' }}</label>
                            <input type="file" name="supporting_documents_attachments[]" id="editSupportingDocumentsAttachments" class="form-control p-3 border border-slate-300 rounded-lg focus:border-blue-500 focus:ring-blue-500" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" multiple>
...
@push('styles')
<style>
    /* Fix for supporting documents filename overflow in vehicle modal */
    ...
    #view-vehicle-modal #vehicle-details .col-span-12 {
        max-width: 100% !important;
        min-width: 0 !important;
    }
    
    #view-vehicle-modal #vehicle-details .grid {
        max-width: 100% !important;
        overflow-x: hidden !important;
    }
    
    #view-vehicle-modal #vehicle-details a {
        max-width: 100% !important;
