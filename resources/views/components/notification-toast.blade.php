...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.showNotification_{{ $toastId }} = function() {
        ...
        try {
            // Close ALL existing toastify notifications (not just ones with 'on' class)
            document.querySelectorAll('.toastify').forEach(el => {
                try {
                    if (el.hideToast && typeof el.hideToast === 'function') {
                        el.hideToast();
                    }
                } catch (e) {}
                el.remove();
            ...
            if (window.__globalActiveToast && typeof window.__globalActiveToast.hideToast === 'function') {
                ...
                window.__globalActiveToast = null;
            }
            // Note: Removed aggressive cleanup that was closing notifications too quickly
        } catch (e) {
        ...
        if (typeof Toastify !== 'undefined' && notificationContent) {
            ...
            setTimeout(() => {
                ...
                if (toastElement) {
                    ...
                    if (profilePicture) {
                        const profileRect = profilePicture.getBoundingClientRect();
                        topValue = profileRect.bottom + 10;
                    } else if (bellIcon) {
                        const bellRect = bellIcon.getBoundingClientRect();
                        topValue = bellRect.bottom + 10;
                    } else {
                        topValue = yOffset;
                    }
                    ...
                    // Also check for any close buttons that might have been added by Toastify
                    setTimeout(() => {
                        const allCloseButtons = toastElement.querySelectorAll('.toast-close');
                    ...
                    // Ensure only one notification is visible - remove any others (but not the current one)
                    setTimeout(() => {
                        document.querySelectorAll('.toastify').forEach(el => {
                            if (el !== toastElement && !el.contains(toastElement)) {
                                try {
                                    if (el.hideToast && typeof el.hideToast === 'function') {
                                        el.hideToast();
                                    }
                    ...
                    }, 200); // Wait a bit to ensure current notification is fully rendered
                }
            }, 100);
