...
<div class="grid grid-cols-12 gap-6 mt-5">
                        ...
                        <tr class="intro-x" data-status="{{ $biz->status }}" data-date="{{ $biz->created_at ? $biz->created_at->format('Y-m-d H:i:s') : '' }}">
                            ...
                            <td class="text-center">
                                @if($biz->business_clearance)
                                    @php
                                        $files = json_decode($biz->business_clearance, true);
                                        $isArray = is_array($files) && count($files) > 0;
                                    @endphp
                                    @if($isArray)
                                        @if(count($files) === 1)
                                            <a href="{{ asset('storage/' . $files[0]) }}" 
                                               target="_blank" 
    ...
    <!-- Upload/Update Application For Certification Button (Bottom) -->
    <div class="intro-y col-span-12 flex justify-center mt-6 mb-4">
        <button class="btn border border-slate-300 bg-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-700" data-tw-toggle="modal" data-tw-target="#upload-certification-template-modal">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
            </svg>
            Upload / Update Application For Certification
        </button>
    </div>
    <!-- END: Upload/Update Application For Certification Button -->
    ...
    <!-- BEGIN: Upload Certification Template Modal -->
    <div id="upload-certification-template-modal" class="modal" data-tw-backdrop="static" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="font-medium text-base mr-auto">Upload / Update Application For Certification</h2>
                ...
                <div class="modal-body px-5 py-10">
                    <style>
                        #upload-certification-template-modal .file-input-wrapper {
                        ...
                        #applicationForCertificationTemplate {
                            width: 100% !important;
                        ...
                        #applicationForCertificationTemplate::before {
                            content: '' !important;
                        ...
                        #applicationForCertificationTemplate::file-selector-button {
                            margin-right: 0.5rem !important;
                        }
                        
                        /* For WebKit browsers (Chrome, Safari, Edge) */
                        #applicationForCertificationTemplate::-webkit-file-upload-button {
                            margin-right: 0.5rem !important;
                        }
                        
                        /* Center the text that appears after file selection */
                        #applicationForCertificationTemplate::after {
                            content: '' !important;
                            display: inline-block !important;
                        }
                        
                        /* Ensure View Current Template button displays icon and text correctly */
                        #upload-certification-template-modal .btn.btn-outline-primary {
                            display: inline-flex !important;
                            align-items: center !important;
                        }
                        
                        #upload-certification-template-modal .btn.btn-outline-primary svg {
                            display: inline-block !important;
                            margin-right: 0.375rem !important;
                            flex-shrink: 0 !important;
                        }
                    </style>
                    <form id="uploadCertificationTemplateForm" enctype="multipart/form-data">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 text-center">
                                <label class="form-label">Select File</label>
                                <div class="file-input-wrapper">
                                    <input type="file" name="application_for_certification_template" id="applicationForCertificationTemplate" class="form-control" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" required>
                                </div>
                                <div class="text-xs text-slate-500 mt-1">This template will be available for all users to download. Accepted formats: PDF, DOC, DOCX, JPG, JPEG, PNG (Max: 10MB)</div>
                                @if($certificationTemplatePath)
                                <div class="mt-3 p-3 border border-green-200 rounded-lg bg-green-50 mx-auto max-w-md">
                                    ...
                                    <div class="flex justify-center">
                                        <a href="{{ route('businessmanagement.download-certification-template') }}?v={{ $certificationTemplatePath ? filemtime($certificationTemplatePath) : time() }}" target="_blank" class="btn btn-outline-primary btn-sm inline-flex items-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                                ...
                                <div class="text-xs text-orange-600 mt-2">
                                    ...
                                    No template uploaded yet. Users will not be able to download the form until a template is uploaded.
                                </div>
                                @endif
                            </div>
                        </div>
                        <div class="flex items-center justify-end mt-5">
                            ...
                            <button type="submit" class="btn btn-primary w-24">
                                ...
                                {{ $certificationTemplatePath ? 'Update' : 'Upload' }}
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <!-- END: Upload Certification Template Modal -->
</div>
@endsection
