...
<div class="intro-y col-span-12">
    ...
    <style>
        ...
        /* Remove red highlight from Tom Select search results */
                    ...
                    <div class="grid grid-cols-12 gap-4 mb-6">
                        <div class="col-span-12">
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <button type="button" id="selectAllUsersBtn" class="btn btn-outline-secondary btn-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                            ...
                            <select name="user_id[]" id="userSelect" data-placeholder="Search and select user(s)..." class="tom-select w-full" multiple required>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" data-role="{{ $user->role }}">{{ $user->name }} ({{ $user->email }})</option>
                                @endforeach
                            </select>
                            <small class="text-slate-500 mt-1">You can select multiple users to create billings for all of them</small>
                        </div>
                    </div>
