...
<html lang="en" class="light">
    ...
    <head>
        ...
        <style>
            ...
            .toastify-content {
                ...
                padding: 1rem 3rem 1rem 1rem !important; /* Extra right padding for close button (3rem for better spacing) */
            ...
            .toastify .toastify-content .toast-close {
                ...
                right: 0.75rem !important; /* Increased from 0.5rem for better spacing */
                top: 50% !important;
            ...
            /* Ensure toasts appear on top right with high z-index - highest priority */
            .toastify,
            .toastify.on {
                z-index: 999999 !important; /* Much higher than topbar z-index of 51 */
            ...
            /* Prevent left positioning from being applied - force all toasts to right */
            .toastify-left,
            ...
            /* Ensure toasts with notification-popup class are on the right */
            .toastify.notification-popup,
            .toastify.toastify-content,
            .toastify .toastify-content,
            .toastify.on {
            ...
            /* Override Toastify default left positioning - force all toasts to right */
            .toastify,
            .toastify.on,
            .toastify.toastify-left,
            ...
            /* Ensure all toasts are positioned on the right by default - highest specificity */
            .toastify.on,
            ...
            /* Override any inline styles that might set left */
            .toastify[style*="left"] {
                left: auto !important;
            }
    ...
    <body class="main">
        ...
        {{-- Database notifications are now handled by the polling system in topbar.blade.php --}}
        {{-- The showToast function in topbar.blade.php will display all notifications --}}
        {{-- (including chatbot notifications) in the top right, below the bell icon and profile picture --}}
        ...
        <script>
        // Override Toastify.reposition to always position toasts on the right
        if (typeof Toastify !== 'undefined' && Toastify.reposition) {
            const originalReposition = Toastify.reposition;
            Toastify.reposition = function() {
                // Call original reposition first
                originalReposition.call(this);
                
                // Then force all toasts to the right and ensure they're on top
                setTimeout(() => {
                    const allToasts = document.querySelectorAll('.toastify.on, .toastify');
                    allToasts.forEach(toastElement => {
                        // Ensure toast is appended to body (not inside any container)
                        if (toastElement.parentElement !== document.body) {
                            document.body.appendChild(toastElement);
                        }
                        
                        // Remove left class and add right class
                        toastElement.classList.remove('toastify-left');
                        toastElement.classList.add('toastify-right');
                        
                        // Force fixed positioning and highest z-index
                        ...
                        // Force right positioning - override any left styles
                        toastElement.style.left = 'auto';
                        toastElement.style.setProperty('left', 'auto', 'important');
                        
                        // Calculate proper right position if we have offset info
                        const hasOffset = toastElement.dataset.xOffset || toastElement.dataset.yOffset;
                        if (!hasOffset) {
                            // Ensure right is set
                            const computedRight = window.getComputedStyle(toastElement).right;
                            if (!computedRight || computedRight === 'auto' || computedRight === '0px') {
                                toastElement.style.right = '20px';
                                toastElement.style.setProperty('right', '20px', 'important');
                            }
                        }
                    });
                }, 10);
        ...
        function openFullChat() {
            // Redirect to full chat page with the current chat user
            if (currentFloatingChatUserId) {
                window.location.href = '/chat?user=' + currentFloatingChatUserId;
            } else {
