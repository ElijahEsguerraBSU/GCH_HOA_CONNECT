<div class="top-bar-boxed h-[70px] z-[51] relative border-b border-white/[0.08] mt-12 md:-mt-5 -mx-3 sm:-mx-8 px-3 sm:px-8 md:pt-0 mb-12">
    <div class="h-full flex items-center">
        <!-- BEGIN: Logo -->
        <a href="{{ route('dashboard') }}" class="-intro-x hidden md:flex items-center hover:opacity-80 transition-opacity">
        ...
        <div class="intro-x dropdown mr-4 sm:mr-6">
            <div class="dropdown-toggle notification cursor-pointer relative" 
                 role="button" aria-expanded="false" data-tw-toggle="dropdown" id="notification-bell"> 
                <i data-lucide="bell" class="notification__icon dark:text-slate-500"></i>
                @if($unreadCount > 0)
                    <span class="notification-badge-dot absolute -top-1 -right-1 w-3 h-3 bg-danger rounded-full border-2 border-white z-10" id="notification-badge"></span>
                @endif
            </div>
...
window.markAllNotificationsAsRead = function() {
    ...
    // Hide the badge immediately (before API call)
    ...
    const badges = document.querySelectorAll('.notification-badge-dot');
    ...
    .then(data => {
        ...
        // Force remove any remaining notification badges
        const allBadges = document.querySelectorAll('.notification-badge-dot, #notification-badge, [id*="notification-badge"]');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Ensure initial badge state reflects server unread at first paint
    try {
        const initialUnread = Number('{{ (int)($unreadCount ?? 0) }}');
        // Wait a bit for updateBell to be available, then set initial state
        setTimeout(() => {
            if (typeof window.updateBell === 'function') {
                window.updateBell(isNaN(initialUnread) ? 0 : initialUnread);
            } else if (typeof updateBell === 'function') {
                updateBell(isNaN(initialUnread) ? 0 : initialUnread);
            }
        }, 100);
    } catch (e) {
        console.error('Error setting initial badge state:', e);
    }
    ...
    document.addEventListener('click', function(e) {
        ...
        try {
            ...
            document.querySelectorAll('.notification-badge-dot').forEach(b => { b.style.display = 'none'; b.remove(); });
    ...
    (function initGlobalNotificationPolling() {
        ...
        function showToast(title, message, type = 'info') {
            ...
            // Copy exact logic from list-payments.js
            ...
            // Copy exact positioning logic from list-payments.js
            setTimeout(() => {
        ...
        function updateBell(count) {
            ...
            console.log('updateBell called with count:', count, 'bellContainer:', bellContainer, 'notificationBell:', notificationBell, 'existing badge:', badge);
            
            if (count > 0) {
                if (bellContainer) {
                    // Ensure relative positioning for badge
                    if (!bellContainer.classList.contains('relative')) {
                        bellContainer.classList.add('relative');
                        console.log('Added relative class to bell container');
                    }
                }
                ...
                if (!badge) {
                    ...
                    badge.className = 'notification-badge-dot absolute -top-1 -right-1 w-3 h-3 bg-danger rounded-full border-2 border-white z-10';
                    
                    // Try to append to notificationBell first, then bellContainer
                    if (notificationBell) {
                        notificationBell.appendChild(badge);
                        console.log('Badge appended to notificationBell');
                    } else if (bellContainer) {
                        bellContainer.appendChild(badge);
                        console.log('Badge appended to bellContainer');
                    } else {
                        console.error('Neither notificationBell nor bellContainer found!');
                    }
                } else {
                    console.log('Badge already exists, showing it');
                    badge.style.display = 'block';
                    badge.style.visibility = 'visible';
                    badge.style.opacity = '1';
                }
            } else {
                console.log('Count is 0, removing badge');
                if (badge) {
                    badge.style.display = 'none';
                    badge.remove();
                }
        ...
        function poll(forceRender = false) {
            ...
            .then(data => {
                if (!data || !data.success) {
                    console.log('Poll: No data or success=false', data);
                    return;
                }
                const list = Array.isArray(data.notifications) ? data.notifications : [];
                const unreadCount = (typeof data.unread_count === 'number') ? data.unread_count : 0;
                console.log('Poll: unreadCount =', unreadCount, 'notifications count =', list.length);
        ...
        // Expose updateBell globally so it can be called from initial state
        window.updateBell = updateBell;
...
.toastify .toastify-content .toast-close {
    ...
    top: 50% !important;
    right: 0.75rem !important; /* Increased spacing from text */
...
.toastify.toastify-content {
    ...
    color: #111827 !important;
    box-shadow: 0 10px 25px rgba(0,0,0,0.12) !important;
    padding: 1rem 3rem 1rem 1rem !important; /* Extra right padding (3rem) for close button spacing */
    position: relative !important;
}
