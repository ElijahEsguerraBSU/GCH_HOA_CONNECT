...
<div class="grid grid-cols-12 gap-6 mt-5">
        ...
        <div class="intro-y col-span-12">
            ...
            <x-notification-toast id="validity_date_toast" type="success" title="Validity Date Set" message="Validity date has been set successfully"
                :showButton="false" />
            <x-notification-toast id="decline_toast" type="success" title="Vehicle Declined" message="Vehicle has been declined successfully"
                :showButton="false" />
            <x-notification-toast id="archived_toast" type="success" title="Vehicle Archived" message="Vehicle has been archived successfully"
                :showButton="false" />
            <style>
                .toastify {
                ...
                /* Force right positioning - override any default left positioning */
                .toastify-right,
                ...
                /* Prevent left positioning from being applied */
                .toastify-left,
                ...
                /* Ensure all toasts are positioned on the right by default */
                .toastify,
                .toastify.on,
                .toastify.on.notification-popup,
                .toastify.on.toastify-content {
                    right: 20px !important;
                    left: auto !important;
        ...
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2 gap-2">
                ...
                <div class="dropdown-menu w-48"> 
                    <ul class="dropdown-content"> 
                        ...
                        @foreach($statuses as $status)
                            ...
                            @endif
                        @endforeach
                        <li> <a href="javascript:;" class="dropdown-item" data-filter-type="status" data-filter-value="Archived">Archived</a> </li> 
                    </ul> 
                </div> 
            </div>
            ...
            <!-- END: Add Vehicle Modal -->
            <div class="hidden md:block mx-auto text-slate-500">Showing <span id="filtered-count">{{ $vehicles->count() }}</span> of <span id="total-count">{{ $vehicles->total() }}</span> entries</div>
                        ...
                        <tr class="intro-x" data-status="{{ $veh->status }}">
                            ...
                            <td class="w-40">
                                <div class="flex items-center justify-center 
                                    @if($veh->status === 'Active' || $veh->status === 'Approved') text-success
                                    @elseif($veh->status === 'Pending') text-warning
                                    @elseif($veh->status === 'Declined') text-danger
                                    @elseif($veh->status === 'Inactive' || $veh->status === 'Archived') text-slate-500
                                    @else text-slate-500
                                    @endif">
                                    @if($veh->status === 'Active' || $veh->status === 'Approved')
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    ...
                                    @if($veh->status === 'Archived')
                                    <a class="flex items-center text-success mr-3" href="javascript:;" data-action="unarchive" data-id="{{ $veh->id }}" data-tw-toggle="modal" data-tw-target="#unarchive-vehicle-modal">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-1">
                                            <path d="M3 3v18h18"></path>
                                            <path d="M18 7l-5-5-5 5"></path>
                                    ...
                                    @endif
                                    @if($veh->status !== 'Archived')
                                    <a class="flex items-center text-warning ml-3" href="javascript:;" data-action="archive" data-id="{{ $veh->id }}" data-tw-toggle="modal" data-tw-target="#archive-vehicle-modal">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"
                                            viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
        ...
        <!-- BEGIN: Archive Modal -->
        <div id="archive-vehicle-modal" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body px-5 py-10">
                        <div class="text-center">
                            <div class="mb-5">Are you sure you want to archive this vehicle? Archived vehicles will be hidden and can only be viewed when the "Archived" filter is selected.</div>
                            <input type="hidden" id="archiveVehicleId" />
                            <button type="button" id="confirmArchiveVehicle" class="btn btn-warning w-24 mr-2">Archive</button>
        ...
        <!-- BEGIN: Unarchive Modal -->
        <div id="unarchive-vehicle-modal" class="modal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body px-5 py-10">
                        <div class="text-center">
                            <div class="mb-5">Are you sure you want to unarchive this vehicle? The vehicle will be restored to "Pending" status.</div>
                            <input type="hidden" id="unarchiveVehicleId" />
                            <button type="button" id="confirmUnarchiveVehicle" class="btn btn-success w-24 mr-2">Unarchive</button>
                            <button type="button" data-tw-dismiss="modal" class="btn btn-outline-secondary w-24">Cancel</button>
                </div>
            </div>
            </div>
                    ...
                    <div class="modal-body px-5 py-10">
                        <div class="text-left">
                            ...
                            <div class="grid grid-cols-12 gap-4">
                                ...
                                <div class="col-span-12">
                                    <label class="form-label">Supporting Documents</label>
                                    <div id="view_supporting_documents" class="form-control bg-slate-50 overflow-hidden p-3" style="word-wrap: break-word; overflow-wrap: break-word; max-width: 100%; width: 100%; box-sizing: border-box;"></div>
                                </div>
                            </div>
        ...
        <!-- BEGIN: Upload Sticker Form Template Modal - Homeowner -->
        <div id="upload-sticker-form-template-homeowner-modal" class="modal" data-tw-backdrop="static" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="font-medium text-base mr-auto">Upload / Update Vehicle Sticker Form Template - Homeowner</h2>
                        <button type="button" data-tw-dismiss="modal" class="btn btn-close">
                    ...
                    <div class="modal-body px-5 py-10">
                        <style>
                            #upload-sticker-form-template-homeowner-modal .file-input-wrapper,
                            #upload-sticker-form-template-nonhomeowner-modal .file-input-wrapper {
                            ...
                            #vehicleStickerFormTemplateHomeowner,
                            #vehicleStickerFormTemplateNonHomeowner {
                                width: 100% !important;
                            ...
                            /* Center the file input's internal text */
                            #vehicleStickerFormTemplateHomeowner::before,
                            #vehicleStickerFormTemplateNonHomeowner::before {
                                content: '' !important;
                                display: inline-block !important;
                                width: 0 !important;
                            }
                            
                            /* For modern browsers - center file selector button */
                            #vehicleStickerFormTemplateHomeowner::file-selector-button,
                            #vehicleStickerFormTemplateNonHomeowner::file-selector-button {
                                margin-right: 0.5rem !important;
                            }
                            
                            /* For WebKit browsers (Chrome, Safari, Edge) */
                            #vehicleStickerFormTemplateHomeowner::-webkit-file-upload-button,
                            #vehicleStickerFormTemplateNonHomeowner::-webkit-file-upload-button {
                                margin-right: 0.5rem !important;
                            }
                            
                            /* Center the text that appears after file selection */
                            #vehicleStickerFormTemplateHomeowner::after,
                            #vehicleStickerFormTemplateNonHomeowner::after {
                                content: '' !important;
                                display: inline-block !important;
                            }
                            
                            /* Ensure View Current Template button displays icon and text correctly */
                            #upload-sticker-form-template-homeowner-modal .btn.btn-outline-primary,
                            #upload-sticker-form-template-nonhomeowner-modal .btn.btn-outline-primary {
                                display: inline-flex !important;
                                align-items: center !important;
                            }
                            
                            #upload-sticker-form-template-homeowner-modal .btn.btn-outline-primary svg,
                            #upload-sticker-form-template-nonhomeowner-modal .btn.btn-outline-primary svg {
                                display: inline-block !important;
                                margin-right: 0.375rem !important;
                                flex-shrink: 0 !important;
                            }
                        </style>
                        <form id="uploadStickerFormTemplateHomeownerForm" enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input type="hidden" name="template_type" value="homeowner">
                            <div class="grid grid-cols-12 gap-4">
                                <div class="col-span-12 text-center">
                                    <label class="form-label">Select File</label>
                                    <div class="file-input-wrapper">
                                        <input type="file" name="vehicle_sticker_form_template" id="vehicleStickerFormTemplateHomeowner" class="form-control" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" required>
                                    </div>
                                    <div class="text-xs text-slate-500 mt-1">This template will be available for homeowners to download. Accepted formats: PDF, DOC, DOCX, JPG, JPEG, PNG (Max: 10MB)</div>
                                    @if($homeownerTemplatePath)
                                    <div class="mt-3 p-3 border border-green-200 rounded-lg bg-green-50 mx-auto max-w-md">
                                        <div class="text-xs text-green-600 mb-2">
                                            ...
                                            Template currently uploaded. Uploading a new file will replace the existing one.
                                        </div>
                                        <div class="flex justify-center">
                                            <a href="{{ route('vehiclemanagement.download-sticker-form-template') }}?type=homeowner&v={{ $homeownerTemplatePath ? filemtime($homeownerTemplatePath) : time() }}" target="_blank" class="btn btn-outline-primary btn-sm inline-flex items-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                                    ...
                                    <div class="text-xs text-orange-600 mt-2">
                                        ...
                                        No template uploaded yet. Homeowners will not be able to download the form until a template is uploaded.
                                    </div>
                                    @endif
                                </div>
                            </div>
                            <div class="flex items-center justify-end mt-5">
                                <button type="button" data-tw-dismiss="modal" class="btn btn-secondary w-24 mr-2">Cancel</button>
                                <button type="submit" class="btn btn-primary w-24">
                                    ...
                                    {{ $homeownerTemplatePath ? 'Update' : 'Upload' }}
                                </button>
                            </div>
                        </form>
                    </div>
        ...
        <!-- END: Upload Sticker Form Template Modal - Homeowner -->

        <!-- BEGIN: Upload Sticker Form Template Modal - Non-Homeowner -->
        <div id="upload-sticker-form-template-nonhomeowner-modal" class="modal" data-tw-backdrop="static" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="font-medium text-base mr-auto">Upload / Update Vehicle Sticker Form Template - Non-Homeowner</h2>
                        <button type="button" data-tw-dismiss="modal" class="btn btn-close">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                <line x1="18" y1="6" x2="6" y2="18"></line>
                                <line x1="6" y1="6" x2="18" y2="18"></line>
                    ...
                    <div class="modal-body px-5 py-10">
                        <form id="uploadStickerFormTemplateNonHomeownerForm" enctype="multipart/form-data">
                            <input type="hidden" name="_token" value="{{ csrf_token() }}">
                            <input type="hidden" name="template_type" value="nonhomeowner">
                            <div class="grid grid-cols-12 gap-4">
                                <div class="col-span-12 text-center">
                                    <label class="form-label">Select File</label>
                                    <div class="file-input-wrapper">
                                        <input type="file" name="vehicle_sticker_form_template" id="vehicleStickerFormTemplateNonHomeowner" class="form-control" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" required>
                                    </div>
                                    <div class="text-xs text-slate-500 mt-1">This template will be available for non-homeowners to download. Accepted formats: PDF, DOC, DOCX, JPG, JPEG, PNG (Max: 10MB)</div>
                                    @if($nonHomeownerTemplatePath)
                                    <div class="mt-3 p-3 border border-green-200 rounded-lg bg-green-50 mx-auto max-w-md">
                                        <div class="text-xs text-green-600 mb-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 inline mr-1">
                                                <polyline points="9 11 12 14 22 4"></polyline>
                                        ...
                                        <div class="flex justify-center">
                                            <a href="{{ route('vehiclemanagement.download-sticker-form-template') }}?type=nonhomeowner&v={{ $nonHomeownerTemplatePath ? filemtime($nonHomeownerTemplatePath) : time() }}" target="_blank" class="btn btn-outline-primary btn-sm inline-flex items-center">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                                                    <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path>
                                                    <polyline points="15 3 21 3 21 9"></polyline>
                                                    <line x1="10" y1="14" x2="21" y2="3"></line>
                                    ...
                                    <div class="text-xs text-orange-600 mt-2">
                                        ...
                                        No template uploaded yet. Non-homeowners will not be able to download the form until a template is uploaded.
                                    </div>
                                    @endif
                                </div>
                            </div>
                            <div class="flex items-center justify-end mt-5">
                                ...
                                <button type="submit" class="btn btn-primary w-24">
                                    ...
                                    {{ $nonHomeownerTemplatePath ? 'Update' : 'Upload' }}
                                </button>
                            </div>
                        </form>
                    </div>
        ...
        <!-- END: Upload Sticker Form Template Modal - Non-Homeowner -->
    <!-- END: Users Layout -->
    ...
    <!-- Vehicle Sticker Form Template Buttons (Bottom) -->
    <div class="intro-y col-span-12 flex justify-center gap-4 mt-6 mb-4">
        <button class="btn border border-slate-300 bg-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-700" data-tw-toggle="modal" data-tw-target="#upload-sticker-form-template-homeowner-modal">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
            </svg>
            Upload / Update Vehicle Sticker Form Template - Homeowner
        </button>
        <button class="btn border border-slate-300 bg-transparent text-slate-600 hover:bg-slate-50 hover:text-slate-700" data-tw-toggle="modal" data-tw-target="#upload-sticker-form-template-nonhomeowner-modal">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path>
                <polyline points="7 10 12 15 17 10"></polyline>
                <line x1="12" y1="15" x2="12" y2="3"></line>
            </svg>
            Upload / Update Vehicle Sticker Form Template - Non-Homeowner
        </button>
    </div>
    <!-- END: Vehicle Sticker Form Template Buttons -->
</div>
...
@push('styles')
<style>
    /* Fix for supporting documents filename overflow in vehicle management modal */
    #view-vehicle-modal .form-control {
    ...
    #view-vehicle-modal #view_supporting_documents {
        max-width: 100% !important;
        width: 100% !important;
        box-sizing: border-box !important;
        overflow: hidden !important;
    ...
    #view-vehicle-modal #view_supporting_documents a {
        max-width: 100% !important;
    ...
    #view-vehicle-modal .col-span-12 {
        max-width: 100% !important;
        min-width: 0 !important;
    }
    
    #view-vehicle-modal .grid {
        max-width: 100% !important;
        overflow-x: hidden !important;
    }
    
    @media (max-width: 768px) {
        #view-vehicle-modal .modal-dialog {
