...
<div class="intro-y col-span-12">
    ...
    <style>
        ...
        /* Receipt Modal - Horizontal Scroll Support */
        #receiptFileDisplay {
        ...
        #receiptFileDisplay .receipt-iframe-container {
            overflow-x: auto !important;
            overflow-y: visible !important;
            -webkit-overflow-scrolling: touch;
            width: 100%;
        ...
        #receiptFileDisplay iframe {
            min-width: 600px;
            display: block;
        }
        
        #receiptFileDisplay img {
            display: block;
            max-width: none;
            width: auto;
            height: auto;
        ...
        /* Receipt Modal Mobile Responsive Styles */
...
<div id="receipt-modal" class="modal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h2 class="font-medium text-base mr-auto" id="receipt-modal-title">Receipt</h2>
                <button type="button" class="btn btn-outline-secondary w-8 h-8 mr-1" data-tw-dismiss="modal">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-x w-4 h-4"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                </button>
            </div>
            <div class="modal-body px-4 sm:px-6 py-4 sm:py-6 overflow-y-auto overflow-x-hidden" style="max-height: calc(100vh - 200px);">
                <div class="mb-4 sm:mb-6" id="receipt-header-info">
                    <div class="bg-blue-50 p-3 sm:p-4 rounded-lg border border-blue-200">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <span class="text-blue-800 font-medium text-sm sm:text-base">Payment Receipt for Bill #<span id="receiptBillNumber"></span></span>
                            <span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-xs sm:text-sm font-medium inline-block w-fit" id="receipt-status">Under Review</span>
                        </div>
                        <div class="text-xs sm:text-sm text-blue-600 mt-2" id="receipt-description">Status: Your payment proof has been submitted and is currently being reviewed.</div>
                    </div>
                </div>

                <div class="receipt-content">
                    <div class="text-center mb-4 sm:mb-6">
                        <h3 class="text-base sm:text-lg font-semibold text-slate-800 mb-2" id="receipt-content-title">Payment Proof</h3>
                        <p class="text-xs sm:text-sm text-slate-500" id="receipt-content-description">Below is the payment proof you uploaded</p>
                    </div>
                    
                    <!-- Receipt Image/PDF Display -->
                    <div class="receipt-display">
                        <div id="receiptFileDisplay" class="w-full overflow-x-auto overflow-y-visible" style="overflow-x: auto; overflow-y: visible; -webkit-overflow-scrolling: touch;">
            ...
            <div class="modal-footer px-4 sm:px-6 py-3 sm:py-4 bg-slate-50 border-t border-slate-200">
                <div class="flex flex-col sm:flex-row justify-between items-stretch sm:items-center w-full gap-2 sm:gap-0">
                    <button type="button" id="downloadReceiptBtn" class="btn btn-outline-primary px-4 sm:px-6 py-2 order-2 sm:order-1">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-4 h-4 mr-2">
                    ...
                    <button type="button" data-tw-dismiss="modal" class="btn btn-secondary px-4 sm:px-6 py-2 order-1 sm:order-2">Close</button>
                </div>
            </div>
        </div>
    </div>
        ...
        function adjustModalLayout() {
            setTimeout(() => {
                ...
                if (filePond && modalBody && modalFooter) {
                    ...
                    const baseModalHeight = viewportHeight * 0.9; // default 90vh
                    let desiredModalHeight = baseModalHeight;
                    ...
                    if (hasPreview) {
                        // Give more space for preview - increase modal height, never smaller than base
                        const previewBuffer = previewHeight + 200;
                        desiredModalHeight = Math.max(baseModalHeight, previewBuffer);
                        desiredModalHeight = Math.min(desiredModalHeight, viewportHeight * 0.95); // cap at 95vh
                        ...
                        console.log('Adjusted layout for preview - Modal height:', desiredModalHeight, 'Preview height:', previewHeight);
                    } else {
                    ...
                    modalContent.style.maxHeight = desiredModalHeight + 'px';
                    
                    console.log('Modal layout adjusted - FilePond height:', filePondHeight, 'Max modal height:', desiredModalHeight);
                }
