    ...
    public function uploadPhoto(Request $request)
    {
        try {
            ...
            if ($request->hasFile('profile_photo')) {
                ...
                // Ensure profiles directory exists on the public disk
                if (!Storage::disk('public')->exists('profiles')) {
                    Storage::disk('public')->makeDirectory('profiles');
                }

                // Store file explicitly on the public disk under profiles
                $storedPath = Storage::disk('public')->putFileAs('profiles', $file, $fileName);

                if (!$storedPath) {
                    return response()->json([
                        'success' => false,
                        'message' => 'Unable to store the uploaded photo. Please check storage permissions.'
                    ], 500);
                }
                ...
                // Reload the user model to ensure fresh data
                $user->refresh();
                
                // Manually construct photo_url to avoid accessor issues on mobile
                $photoUrl = asset('storage/profiles/' . $fileName);
    ...
    public function addTenant(Request $request)
    {
        try {
            ...
            if ($request->hasFile('photo')) {
                ...
                // Store in storage/app/public/tenants using the public disk
                $file->storeAs('tenants', $fileName, 'public');
    ...
    public function updateTenant(Request $request, $id)
    {
        try {
            ...
            if ($request->hasFile('photo')) {
                ...
                // Delete old photo if it exists
                if ($tenant->photo && Storage::disk('public')->exists('tenants/' . $tenant->photo)) {
                    Storage::disk('public')->delete('tenants/' . $tenant->photo);
                }
