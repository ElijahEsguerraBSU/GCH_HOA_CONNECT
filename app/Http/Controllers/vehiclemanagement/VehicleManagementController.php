...
class VehicleManagementController extends Controller
{
    ...
    public function downloadStickerFormTemplate(Request $request)
    {
        try {
            ...
            // Get the directory path from storage/vehicle_templates
            $directoryPath = storage_path('vehicle_templates');
    ...
    public function approve($id)
    {
        try {
            ...
            // Refresh vehicle with relationships to get the control number
            $vehicle->refresh();
            $vehicle->load(['supportingDocuments.vehicleDetails.stickerControl']);
            
            // Send notification to vehicle (vehicle) users
            $this->sendVehicleNotifications($vehicle, 'approved');
    ...
    public function decline(Request $request, $id)
    {
        try {
            ...
            // Send notification to vehicle (vehicle) users
            $this->sendVehicleNotifications($vehicle, 'declined', $validated['reason']);
    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'supporting_documents_attachments.*' => 'nullable|file|mimes:pdf,doc,docx,jpg,jpeg,png|max:15360'
            ]);
    ...
    /**
     * Get users who have notification settings for vehicle (vehicle)
     */
    private function getVehicleNotificationUsers()
    {
        try {
            // Find the vehicle module (vehicle module)
            $vehicleModule = module::where('module_name', 'vehicle')
                ->where('status', 'active')
                ->first();
            
            if (!$vehicleModule) {
                \Log::warning('Vehicle module not found or inactive');
                return collect();
            }

            // Get users with active notification settings for vehicle
            $notificationSettings = notification_settings::with('user')
                ->where('module_id', $vehicleModule->id)
                ->where('status', 'active')
                ->get();
            ...
            \Log::error('Error getting vehicle notification users: ' . $e->getMessage());
            return collect();
        }
    }

    /**
     * Send notification to vehicle (vehicle) users when a vehicle is updated
     * Only notifies the request owner (the user who submitted the vehicle)
     */
    private function sendVehicleNotifications($vehicle, $action, $reason = null)
    {
        try {
            // Load relationships for message (include stickerControl for approved vehicles)
            $vehicle->load(['user', 'supportingDocuments.vehicleDetails.stickerControl']);
            ...
            $requestOwner = User::find($vehicle->user_id);
            if (!$requestOwner) {
            ...
            // Get vehicle module ID
            $vehicleModule = module::where('module_name', 'vehicle')
                ->where('status', 'active')
                ->first();
            $moduleId = $vehicleModule ? $vehicleModule->id : null;

            $vehicleType = $vehicle->type_of_vehicle ?? 'N/A';
            $plateNumber = $vehicle->supportingDocuments->vehicleDetails->plate_number ?? 'N/A';
            ...
            if ($action === 'approved') {
                ...
                $controlNumber = $vehicle->supportingDocuments->vehicleDetails->stickerControl->control_number ?? 'N/A';
                $message = "Your vehicle registration has been approved. Type: " . $vehicleType . ". Plate Number: " . $plateNumber . ". Control Number: " . $controlNumber . ". Status: " . $currentStatus;
            } elseif ($action === 'declined') {
                $title = 'Vehicle Registration Declined';
                $message = "Your vehicle registration has been declined. Type: " . $vehicleType . ". Plate Number: " . $plateNumber . ". ";
                if ($reason) {
            ...
            try {
                ...
                \Log::info('Vehicle notification sent to request owner', [
            ...
            \Log::info('Vehicle notification completed', [
                'vehicle_id' => $vehicle->id,
                'action' => $action,
            ...
            \Log::error('Error sending vehicle notifications: ' . $e->getMessage(), [
