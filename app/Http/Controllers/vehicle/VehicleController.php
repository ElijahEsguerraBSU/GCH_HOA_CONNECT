...
class VehicleController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'supporting_documents_attachments.*' => 'required|file|mimes:pdf,doc,docx,jpg,jpeg,png|max:15360'
            ]);
            ...
            // Send notification to vehicle-management users
            $this->sendVehicleManagementNotifications($vehicleHomeowner);
    ...
    /**
     * Get users who have notification settings for vehicle management
     */
    private function getVehicleManagementNotificationUsers()
    {
        try {
            // Find the vehicle management module
            $vehicleManagementModule = module::where('module_name', 'vehicle management')
                ->where('status', 'active')
                ->first();
            
            if (!$vehicleManagementModule) {
                Log::warning('Vehicle management module not found or inactive');
                return collect();
            }

            // Get users with active notification settings for vehicle management
            $notificationSettings = notification_settings::with('user')
                ->where('module_id', $vehicleManagementModule->id)
                ->where('status', 'active')
                ->get();
            ...
            Log::error('Error getting vehicle management notification users: ' . $e->getMessage());
            return collect();
        }
    }

    /**
     * Send notification to vehicle-management users when a vehicle is created
     */
    private function sendVehicleManagementNotifications($vehicle)
    {
        try {
            $users = $this->getVehicleManagementNotificationUsers();
            
            if ($users->isEmpty()) {
                Log::info('No vehicle management notification users found');
                return;
            }

            // Load relationships for message
            $vehicle->load(['user', 'supportingDocuments.vehicleDetails']);

            // Get vehicle management module ID
            $vehicleManagementModule = module::where('module_name', 'vehicle management')
                ->where('status', 'active')
                ->first();
            $moduleId = $vehicleManagementModule ? $vehicleManagementModule->id : null;

            $vehicleType = $vehicle->type_of_vehicle ?? 'N/A';
            $plateNumber = $vehicle->supportingDocuments->vehicleDetails->plate_number ?? 'N/A';
            $owner = $vehicle->supportingDocuments->vehicleDetails->owner ?? 'N/A';
            $userName = $vehicle->user->name ?? 'N/A';

            $notificationsSent = 0;
            foreach ($users as $user) {
                // Skip the user who created the vehicle
                if ($user->id === Auth::id()) {
                    continue;
                }
                ...
                try {
                    $notification = $user->notifyInfo(
                        ...
                        Auth::user()->name . " has submitted a new vehicle registration. Type: " . $vehicleType . ". Plate Number: " . $plateNumber . ". Owner: " . $owner . ". Status: " . ($vehicle->status ?? 'Pending'),
                        $moduleId
                    );

                    Log::info('Vehicle management notification sent successfully', [
            ...
            Log::info('Vehicle management notifications completed', [
                'vehicle_id' => $vehicle->id,
            ...
            Log::error('Error sending vehicle management notifications: ' . $e->getMessage(), [
